...
/* loaded from: cglib-3.1.jar:net/sf/cglib/core/TypeUtils.class */
public class TypeUtils {
    ...
    public static Type getType(String className) {
        return Type.getType(new StringBuffer().append(StandardRoles.L).append(className.replace('.', '/')).append(ScriptUtils.DEFAULT_STATEMENT_SEPARATOR).toString());
    }
    ...
    public static String getClassName(Type type) {
        ...
        if (isArray(type)) {
            return new StringBuffer().append(getClassName(getComponentType(type))).append("[]").toString();
        }
    ...
    public static Type fromInternalName(String name) {
        return Type.getType(new StringBuffer().append(StandardRoles.L).append(name).append(ScriptUtils.DEFAULT_STATEMENT_SEPARATOR).toString());
    }
    ...
    public static Signature parseConstructor(String sig) {
        return parseSignature(new StringBuffer().append("void <init>(").append(sig).append(")").toString());
    }
    ...
    private static String map(String type) {
        ...
        if (type.indexOf(46) < 0) {
            return map(new StringBuffer().append("java.lang.").append(type).toString());
        }
        StringBuffer sb = new StringBuffer();
        int index = 0;
        while (true) {
