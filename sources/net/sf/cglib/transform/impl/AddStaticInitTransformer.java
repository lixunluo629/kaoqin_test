...
/* loaded from: cglib-3.1.jar:net/sf/cglib/transform/impl/AddStaticInitTransformer.class */
public class AddStaticInitTransformer extends ClassEmitterTransformer {
    ...
    public AddStaticInitTransformer(Method classInit) {
        ...
        if (!TypeUtils.isStatic(this.info.getModifiers())) {
            throw new IllegalArgumentException(new StringBuffer().append(classInit).append(" is not static").toString());
        }
        ...
        if (types.length != 1 || !types[0].equals(Constants.TYPE_CLASS) || !this.info.getSignature().getReturnType().equals(Type.VOID_TYPE)) {
            throw new IllegalArgumentException(new StringBuffer().append(classInit).append(" illegal signature").toString());
        }
    }
