...
/* loaded from: springfox-swagger2-2.2.2.jar:springfox/documentation/swagger2/mappers/ServiceModelToSwagger2Mapper.class */
public abstract class ServiceModelToSwagger2Mapper {
    @Mappings({@Mapping(target = Protocol.CLUSTER_INFO, source = "resourceListing.info"), @Mapping(target = "paths", source = "apiListings"), @Mapping(target = "schemes", source = "from.schemes"), @Mapping(target = "definitions", source = "apiListings"), @Mapping(target = "securityDefinitions", source = "resourceListing"), @Mapping(target = "securityRequirement", ignore = true), @Mapping(target = "security", ignore = true), @Mapping(target = "swagger", ignore = true), @Mapping(target = "parameters", ignore = true), @Mapping(target = "host", ignore = true), @Mapping(target = "externalDocs", ignore = true)})
    public abstract Swagger mapDocumentation(Documentation documentation);

    @Mappings({@Mapping(target = "license", source = "from", qualifiedBy = {LicenseMapper.LicenseTranslator.class, LicenseMapper.License.class}), @Mapping(target = "contact", source = "from.contact"), @Mapping(target = "termsOfService", source = "termsOfServiceUrl"), @Mapping(target = "vendorExtensions", ignore = true)})
    protected abstract Info mapApiInfo(ApiInfo apiInfo);

    @Mappings({@Mapping(target = "description", source = "notes"), @Mapping(target = "operationId", source = "uniqueId"), @Mapping(target = "schemes", source = "protocol"), @Mapping(target = "security", source = "securityReferences"), @Mapping(target = "responses", source = "responseMessages"), @Mapping(target = "vendorExtensions", source = "vendorExtensions"), @Mapping(target = "externalDocs", ignore = true)})
    protected abstract Operation mapOperation(springfox.documentation.service.Operation operation);

    @Mappings({@Mapping(target = "externalDocs", ignore = true), @Mapping(target = "vendorExtensions", ignore = true)})
    ...
    protected List<Map<String, List<String>>> mapAuthorizations(Map<String, List<AuthorizationScope>> from) {
        List<Map<String, List<String>>> security = Lists.newArrayList();
        for (Map.Entry<String, List<AuthorizationScope>> each : from.entrySet()) {
            Map<String, List<String>> newEntry = Maps.newHashMap();
            newEntry.put(each.getKey(), FluentIterable.from(each.getValue()).transform(scopeToString()).toList());
    ...
    protected Map<String, Response> mapResponseMessages(Set<ResponseMessage> from) {
        HashMap<String, Response> responses = Maps.newHashMap();
    ...
    private Function<AuthorizationScope, String> scopeToString() {
        return new Function<AuthorizationScope, String>() { // from class: springfox.documentation.swagger2.mappers.ServiceModelToSwagger2Mapper.1
    ...
    private Path mapOperations(ApiDescription api, Optional<Path> existingPath) {
        Path path = existingPath.or((Optional<Path>) new Path());
    ...
    private Function<String, Scheme> toScheme() {
        return new Function<String, Scheme>() { // from class: springfox.documentation.swagger2.mappers.ServiceModelToSwagger2Mapper.2
            @Override // com.google.common.base.Function
