...
/* loaded from: netty-all-4.1.50.Final.jar:io/netty/channel/DefaultFileRegion.class */
public class DefaultFileRegion extends AbstractReferenceCounted implements FileRegion {
    ...
    public DefaultFileRegion(File f, long position, long count) {
        this.f = (File) ObjectUtil.checkNotNull(f, ExcelXmlConstants.CELL_FORMULA_TAG);
        this.position = ObjectUtil.checkPositiveOrZero(position, "position");
        this.count = ObjectUtil.checkPositiveOrZero(count, "count");
    }
    ...
    public void open() throws IOException {
        if (!isOpen() && refCnt() > 0) {
            this.file = new RandomAccessFile(this.f, ExcelXmlConstants.POSITION).getChannel();
        }
    }
