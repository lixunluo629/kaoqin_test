...
/* loaded from: jedis-2.9.3.jar:redis/clients/jedis/PipelineBase.class */
public abstract class PipelineBase extends Queable implements BinaryRedisPipeline, RedisPipeline {
    protected abstract Client getClient(String str);

    protected abstract Client getClient(byte[] bArr);

    @Override // redis.clients.jedis.RedisPipeline
    public Response<Long> append(String key, String value) {
        getClient(key).append(key, value);
        return getResponse(BuilderFactory.LONG);
    }

    @Override // redis.clients.jedis.BinaryRedisPipeline
    ...
    public Response<List<byte[]>> blpop(byte[] key) {
        getClient(key).blpop((byte[][]) new byte[]{key});
    ...
    public Response<List<byte[]>> brpop(byte[] key) {
        getClient(key).brpop((byte[][]) new byte[]{key});
        return getResponse(BuilderFactory.BYTE_ARRAY_LIST);
    }
    ...
    public Response<Long> del(byte[] key) {
        getClient(key).del((byte[][]) new byte[]{key});
        return getResponse(BuilderFactory.LONG);
    }
    ...
    public Response<Boolean> exists(byte[] key) {
        getClient(key).exists((byte[][]) new byte[]{key});
        return getResponse(BuilderFactory.BOOLEAN);
    }
    ...
    @Override // redis.clients.jedis.BinaryRedisPipeline, redis.clients.jedis.RedisPipeline
    ...
    public Response<Long> zremrangeByScore(byte[] key, double min, double max) {
        getClient(key).zremrangeByScore(key, Protocol.toByteArray(min), Protocol.toByteArray(max));
        return getResponse(BuilderFactory.LONG);
    }
