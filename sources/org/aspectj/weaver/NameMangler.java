...
/* loaded from: aspectjweaver-1.8.14.jar:org/aspectj/weaver/NameMangler.class */
public class NameMangler {
    ...
    public static boolean isSyntheticMethod(String methodName, boolean declaredInAspect) {
        if (methodName.startsWith(PREFIX)) {
            ...
            if (methodName.startsWith("ajc$interMethod$")) {
                return false;
            }
            return true;
        }
        if (methodName.indexOf("_aroundBody") != -1) {
            return true;
        }
        return false;
    }
    ...
    public static String superDispatchMethod(UnresolvedType classType, String name) {
        return makeName(ResolvedTypeMunger.SUPER_DISPATCH_NAME, classType.getNameAsIdentifier(), name);
    }
    ...
    private static String makeName(String s1, String s2) {
        return PREFIX + s1 + PropertiesBeanDefinitionReader.CONSTRUCTOR_ARG_PREFIX + s2;
    }

    public static String makeName(String s1, String s2, String s3) {
        return PREFIX + s1 + PropertiesBeanDefinitionReader.CONSTRUCTOR_ARG_PREFIX + s2 + PropertiesBeanDefinitionReader.CONSTRUCTOR_ARG_PREFIX + s3;
    }

    public static String makeName(String s1, String s2, String s3, String s4) {
        return PREFIX + s1 + PropertiesBeanDefinitionReader.CONSTRUCTOR_ARG_PREFIX + s2 + PropertiesBeanDefinitionReader.CONSTRUCTOR_ARG_PREFIX + s3 + PropertiesBeanDefinitionReader.CONSTRUCTOR_ARG_PREFIX + s4;
    }
