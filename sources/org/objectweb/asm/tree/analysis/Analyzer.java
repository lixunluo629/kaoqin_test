...
/* loaded from: lombok-1.16.22.jar:org/objectweb/asm/tree/analysis/Analyzer.SCL.lombok */
public class Analyzer<V extends Value> implements Opcodes {
        ...
        while (this.numInstructionsToProcess > 0) {
            ...
            try {
                ...
                if (insnType == 8 || insnType == 15 || insnType == 14) {
                    ...
                    if (insnNode instanceof JumpInsnNode) {
                        ...
                        for (int i4 = 0; i4 < lookupSwitchInsn.labels.size(); i4++) {
                            int targetInsnIndex2 = this.insnList.indexOf(lookupSwitchInsn.labels.get(i4));
                        ...
                        for (int i5 = 0; i5 < tableSwitchInsn.labels.size(); i5++) {
                            int targetInsnIndex4 = this.insnList.indexOf(tableSwitchInsn.labels.get(i5));
                ...
                if (insnHandlers2 != null) {
                    for (int i7 = 0; i7 < insnHandlers2.size(); i7++) {
                        TryCatchBlockNode tryCatchBlock2 = insnHandlers2.get(i7);
                ...
                throw new AnalyzerException(insnNode, "Error at instruction " + insnIndex + ": " + e2.getMessage(), e2);
            }
        }
        return this.frames;
    }
    ...
    private void findSubroutine(int insnIndex, Subroutine subroutine, List<AbstractInsnNode> jsrInsns) throws AnalyzerException {
        for (int currentInsnIndex = insnIndex; currentInsnIndex >= 0 && currentInsnIndex < this.insnListSize; currentInsnIndex++) {
            ...
            if (currentInsn instanceof JumpInsnNode) {
                if (currentInsn.getOpcode() == 168) {
                    ...
                    findSubroutine(this.insnList.indexOf(jumpInsn.label), subroutine, jsrInsns);
                }
                ...
                for (int i = tableSwitchInsn.labels.size() - 1; i >= 0; i--) {
                    ...
                    findSubroutine(this.insnList.indexOf(l), subroutine, jsrInsns);
                }
                ...
                for (int i2 = lookupSwitchInsn.labels.size() - 1; i2 >= 0; i2--) {
                    LabelNode l2 = lookupSwitchInsn.labels.get(i2);
                    findSubroutine(this.insnList.indexOf(l2), subroutine, jsrInsns);
                }
            }
            List<TryCatchBlockNode> insnHandlers = this.handlers[currentInsnIndex];
            if (insnHandlers != null) {
                for (int i3 = 0; i3 < insnHandlers.size(); i3++) {
                    TryCatchBlockNode tryCatchBlock = insnHandlers.get(i3);
                    findSubroutine(this.insnList.indexOf(tryCatchBlock.handler), subroutine, jsrInsns);
                }
            }
    ...
    private Frame<V> computeInitialFrame(String owner, MethodNode method) {
        ...
        if ((method.access & 8) == 0) {
            ...
            currentLocal = 0 + 1;
            frame.setLocal(0, this.interpreter.newValue(ownerType));
        }
        Type[] argumentTypes = Type.getArgumentTypes(method.desc);
        for (int i = 0; i < argumentTypes.length; i++) {
            int i2 = currentLocal;
            currentLocal++;
            frame.setLocal(i2, this.interpreter.newValue(argumentTypes[i]));
        ...
        while (currentLocal < method.maxLocals) {
            int i3 = currentLocal;
            currentLocal++;
            frame.setLocal(i3, this.interpreter.newValue(null));
        }
