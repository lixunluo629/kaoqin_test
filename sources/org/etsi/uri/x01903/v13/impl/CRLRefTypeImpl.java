...
/* loaded from: poi-ooxml-schemas-3.17.jar:org/etsi/uri/x01903/v13/impl/CRLRefTypeImpl.class */
public class CRLRefTypeImpl extends XmlComplexContentImpl implements CRLRefType {
    ...
    public void setDigestAlgAndValue(DigestAlgAndValueType digestAlgAndValueType) {
        synchronized (monitor()) {
            check_orphaned();
            DigestAlgAndValueType digestAlgAndValueType2 = (DigestAlgAndValueType) get_store().find_element_user(DIGESTALGANDVALUE$0, 0);
            if (digestAlgAndValueType2 == null) {
                digestAlgAndValueType2 = (DigestAlgAndValueType) get_store().add_element_user(DIGESTALGANDVALUE$0);
            }
    ...
    public void setCRLIdentifier(CRLIdentifierType cRLIdentifierType) {
        synchronized (monitor()) {
            check_orphaned();
            CRLIdentifierType cRLIdentifierType2 = (CRLIdentifierType) get_store().find_element_user(CRLIDENTIFIER$2, 0);
            if (cRLIdentifierType2 == null) {
                cRLIdentifierType2 = (CRLIdentifierType) get_store().add_element_user(CRLIDENTIFIER$2);
            }
