...
/* loaded from: poi-ooxml-schemas-3.17.jar:org/w3/x2000/x09/xmldsig/impl/TransformTypeImpl.class */
public class TransformTypeImpl extends XmlComplexContentImpl implements TransformType {
    ...
    public int sizeOfXPathArray() {
        ...
        synchronized (monitor()) {
            check_orphaned();
            iCount_elements = get_store().count_elements(XPATH$0);
        }
    ...
    public void setAlgorithm(String str) {
        synchronized (monitor()) {
            ...
            SimpleValue simpleValue = (SimpleValue) get_store().find_attribute_user(ALGORITHM$2);
            if (simpleValue == null) {
                simpleValue = (SimpleValue) get_store().add_attribute_user(ALGORITHM$2);
            }
            simpleValue.setStringValue(str);
        }
    }
    ...
    public void xsetAlgorithm(XmlAnyURI xmlAnyURI) {
        synchronized (monitor()) {
            check_orphaned();
            XmlAnyURI xmlAnyURI2 = (XmlAnyURI) get_store().find_attribute_user(ALGORITHM$2);
            if (xmlAnyURI2 == null) {
                xmlAnyURI2 = (XmlAnyURI) get_store().add_attribute_user(ALGORITHM$2);
            }
