...
/* loaded from: tomcat-embed-websocket-8.5.43.jar:org/apache/tomcat/websocket/Util.class */
public class Util {
    ...
    public static List<DecoderEntry> getDecoders(List<Class<? extends Decoder>> decoderClazzes) throws IllegalAccessException, InstantiationException, IllegalArgumentException, DeploymentException, InvocationTargetException {
    ...
    static Set<MessageHandlerResult> getMessageHandlers(Class<?> target, MessageHandler listener, EndpointConfig endpointConfig, Session session) {
        ...
        if (String.class.isAssignableFrom(target)) {
            ...
            MessageHandlerResult result4 = new MessageHandlerResult(whole ? new PojoMessageHandlerWholeBinary(listener, getOnMessageMethod(listener), session, endpointConfig, matchDecoders(target, endpointConfig, true), new Object[1], 0, true, -1, false, -1L) : new PojoMessageHandlerPartialBinary(listener, getOnMessagePartialMethod(listener), session, new Object[2], 0, true, 1, -1, -1L), MessageHandlerResultType.BINARY);
            results.add(result4);
    ...
    private static List<Class<? extends Decoder>> matchDecoders(Class<?> target, EndpointConfig endpointConfig, boolean binary) throws IllegalAccessException, InstantiationException, IllegalArgumentException, InvocationTargetException {
        DecoderMatch decoderMatch = matchDecoders(target, endpointConfig);
        if (binary) {
            if (decoderMatch.getBinaryDecoders().size() > 0) {
    ...
    private static DecoderMatch matchDecoders(Class<?> target, EndpointConfig endpointConfig) throws IllegalAccessException, InstantiationException, IllegalArgumentException, InvocationTargetException {
        try {
    ...
    public static void parseExtensionHeader(List<Extension> extensions, String header) {
        ...
        for (String unparsedExtension : unparsedExtensions) {
            String[] unparsedParameters = unparsedExtension.split(ScriptUtils.DEFAULT_STATEMENT_SEPARATOR);
    ...
    /* loaded from: tomcat-embed-websocket-8.5.43.jar:org/apache/tomcat/websocket/Util$DecoderMatch.class */
    ...
    /* loaded from: tomcat-embed-websocket-8.5.43.jar:org/apache/tomcat/websocket/Util$TypeResult.class */
