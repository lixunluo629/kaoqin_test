...
/* loaded from: tomcat-embed-core-8.5.43.jar:org/apache/tomcat/util/modeler/modules/MbeansDescriptorsIntrospectionSource.class */
            ...
            if (!Modifier.isStatic(methods[j].getModifiers())) {
                if (!Modifier.isPublic(methods[j].getModifiers())) {
                    ...
                    if (name.startsWith(BeanUtil.PREFIX_GETTER_GET) && params.length == 0) {
                        ...
                        if (Boolean.TYPE != ret) {
                            if (log.isDebugEnabled()) {
                                log.debug("Unsupported type " + methods[j] + SymbolConstants.SPACE_SYMBOL + ret);
                            }
                        } else {
                        ...
                        if (!supportedType(params[0])) {
                            if (log.isDebugEnabled()) {
                                log.debug("Unsupported type " + methods[j] + SymbolConstants.SPACE_SYMBOL + params[0]);
                            }
                        } else {
    ...
    public ManagedBean createManagedBean(Registry registry, String domain, Class<?> realClass, String type) throws SecurityException {
        ...
        try {
            ...
            while (en.hasMoreElements()) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Introspected attribute " + name + SymbolConstants.SPACE_SYMBOL + gm + SymbolConstants.SPACE_SYMBOL + sm);
                }
                if (gm == null) {
