...
/* loaded from: tomcat-embed-core-8.5.43.jar:org/apache/catalina/realm/JNDIRealm.class */
public class JNDIRealm extends RealmBase {
    ...
    protected List<String> getRoles(DirContext context, User user) throws NamingException {
        ...
        if (this.roleBaseFormat != null) {
            ...
            Name name2 = np.parse(dn);
            String[] nameParts = new String[name2.size()];
            for (int i = 0; i < name2.size(); i++) {
                nameParts[i] = name2.get(i);
            }
            ...
            try {
                try {
                    ...
                    if (attrs != null) {
                        ...
                        String name3 = getAttributeValue(this.roleName, attrs);
                        if (name3 != null && dname != null) {
                            groupMap.put(dname, name3);
                        }
                    }
                } catch (PartialResultException ex) {
                    if (!this.adCompat) {
                        ...
                        try {
                            try {
                                ...
                                if (attrs2 != null) {
                                    ...
                                    String name4 = getAttributeValue(this.roleName, attrs2);
                                    if (name4 != null && dname2 != null && !groupMap.keySet().contains(dname2)) {
                                        groupMap.put(dname2, name4);
                                        newThisRound.put(dname2, name4);
                                        if (this.containerLog.isTraceEnabled()) {
                                            this.containerLog.trace("  Found nested role " + dname2 + " -> " + name4);
                                        }
                                    }
                                }
                            } catch (PartialResultException ex2) {
    ...
    protected String getDistinguishedName(DirContext context, String base, SearchResult result) throws InvalidNameException, NamingException {
        ...
        if (result.isRelative()) {
            ...
            Name name3 = contextName.addAll(baseName);
            name2 = name3.addAll(entryName);
        } else {
            if (this.containerLog.isTraceEnabled()) {
            ...
            try {
                ...
                name2 = parser2.parse(pathComponent.substring(1));
            } catch (URISyntaxException e) {
                throw new InvalidNameException("Search returned unparseable absolute name: " + resultName);
            }
        }
        if (getForceDnHexEscape()) {
            return convertToHexEscape(name2.toString());
        }
    ...
    /* loaded from: tomcat-embed-core-8.5.43.jar:org/apache/catalina/realm/JNDIRealm$User.class */
