...
/* loaded from: tomcat-embed-core-8.5.43.jar:org/apache/catalina/realm/RealmBase.class */
public abstract class RealmBase extends LifecycleMBeanBase implements Realm {
    ...
    public SecurityConstraint[] findSecurityConstraints(Request request, Context context) {
        ...
        for (int i = 0; i < constraints.length; i++) {
            ...
            if (collection != null) {
                if (log.isDebugEnabled()) {
                    log.debug("  Checking constraint '" + constraints[i] + "' against " + method + SymbolConstants.SPACE_SYMBOL + uri + " --> " + constraints[i].included(uri, method));
                }
        ...
        for (int i2 = 0; i2 < constraints.length; i2++) {
            ...
            if (collection2 != null) {
                if (log.isDebugEnabled()) {
                    log.debug("  Checking constraint '" + constraints[i2] + "' against " + method + SymbolConstants.SPACE_SYMBOL + uri + " --> " + constraints[i2].included(uri, method));
                }
                for (int j2 = 0; j2 < collection2.length; j2++) {
                    ...
                    if (patterns2 != null) {
                        ...
                        for (String pattern : patterns2) {
                            if (pattern.startsWith("/") && pattern.endsWith(ScriptUtils.DEFAULT_BLOCK_COMMENT_START_DELIMITER) && pattern.length() >= longest) {
                                if (pattern.length() == 2) {
                                    matched = true;
        ...
        for (int i3 = 0; i3 < constraints.length; i3++) {
            ...
            if (collection3 != null) {
                if (log.isDebugEnabled()) {
                    log.debug("  Checking constraint '" + constraints[i3] + "' against " + method + SymbolConstants.SPACE_SYMBOL + uri + " --> " + constraints[i3].included(uri, method));
                }
        ...
        for (int i4 = 0; i4 < constraints.length; i4++) {
            ...
            if (collection4 != null) {
                if (log.isDebugEnabled()) {
                    log.debug("  Checking constraint '" + constraints[i4] + "' against " + method + SymbolConstants.SPACE_SYMBOL + uri + " --> " + constraints[i4].included(uri, method));
                }
    ...
    public boolean hasUserDataPermission(Request request, Response response, SecurityConstraint[] constraints) throws IOException {
        ...
        if (requestedSessionId != null && request.isRequestedSessionIdFromURL()) {
            file.append(ScriptUtils.DEFAULT_STATEMENT_SEPARATOR);
            file.append(SessionConfig.getSessionUriParamName(request.getContext()));
            file.append(SymbolConstants.EQUAL_SYMBOL);
    ...
    /* loaded from: tomcat-embed-core-8.5.43.jar:org/apache/catalina/realm/RealmBase$AllRolesMode.class */
