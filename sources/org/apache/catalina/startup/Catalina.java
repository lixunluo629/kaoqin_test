...
/* loaded from: tomcat-embed-core-8.5.43.jar:org/apache/catalina/startup/Catalina.class */
public class Catalina {
    ...
    protected boolean arguments(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (isConfig) {
                ...
                if (!args[i].equals(Lifecycle.START_EVENT) && !args[i].equals("configtest") && !args[i].equals(Lifecycle.STOP_EVENT)) {
                    usage();
                    return false;
                }
            }
    ...
    public void stopServer(String[] arguments) throws IOException {
        ...
        try {
            FileInputStream fis = new FileInputStream((File) ConfigFile);
            Throwable th = null;
            try {
                InputSource is = new InputSource(ConfigFile.toURI().toURL().toString());
                is.setByteStream(fis);
                CreateStopDigester.push(this);
                CreateStopDigester.parse(is);
                if (fis != null) {
                    if (0 != 0) {
                        try {
                            fis.close();
    ...
    public void load() throws NoSuchMethodException, ClassNotFoundException, SecurityException, IOException {
        if (this.loaded) {
            return;
        }
        this.loaded = true;
    ...
    public void start() throws NoSuchMethodException, ClassNotFoundException, SecurityException, IOException {
        if (getServer() == null) {
            load();
        }
        if (getServer() == null) {
    ...
    /* loaded from: tomcat-embed-core-8.5.43.jar:org/apache/catalina/startup/Catalina$CatalinaShutdownHook.class */
