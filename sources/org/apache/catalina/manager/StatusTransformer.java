...
/* loaded from: tomcat-embed-core-8.5.43.jar:org/apache/catalina/manager/StatusTransformer.class */
public class StatusTransformer {
    ...
    public static void writeOSState(PrintWriter writer, int mode, Object[] args) {
        ...
        try {
            ...
            Method method = Class.forName("org.apache.tomcat.jni.OS").getMethod(Protocol.CLUSTER_INFO, paramTypes);
    ...
    public static void writeConnectorState(PrintWriter writer, ObjectName tpName, String name, MBeanServer mBeanServer, Vector<ObjectName> globalRequestProcessors, Vector<ObjectName> requestProcessors, int mode, Object[] args) throws Exception {
        if (mode != 0) {
            if (mode == 1) {
                ...
                writer.write(" maxThreads=\"" + mBeanServer.getAttribute(tpName, "maxThreads") + SymbolConstants.QUOTES_SYMBOL);
                writer.write(" currentThreadCount=\"" + mBeanServer.getAttribute(tpName, "currentThreadCount") + SymbolConstants.QUOTES_SYMBOL);
                writer.write(" currentThreadsBusy=\"" + mBeanServer.getAttribute(tpName, "currentThreadsBusy") + SymbolConstants.QUOTES_SYMBOL);
                writer.write(" />");
                ...
                if (grpName != null) {
                    ...
                    writer.write(" maxTime=\"" + mBeanServer.getAttribute(grpName, "maxTime") + SymbolConstants.QUOTES_SYMBOL);
                    writer.write(" processingTime=\"" + mBeanServer.getAttribute(grpName, "processingTime") + SymbolConstants.QUOTES_SYMBOL);
                    writer.write(" requestCount=\"" + mBeanServer.getAttribute(grpName, "requestCount") + SymbolConstants.QUOTES_SYMBOL);
                    writer.write(" errorCount=\"" + mBeanServer.getAttribute(grpName, "errorCount") + SymbolConstants.QUOTES_SYMBOL);
                    writer.write(" bytesReceived=\"" + mBeanServer.getAttribute(grpName, "bytesReceived") + SymbolConstants.QUOTES_SYMBOL);
                    writer.write(" bytesSent=\"" + mBeanServer.getAttribute(grpName, "bytesSent") + SymbolConstants.QUOTES_SYMBOL);
                    writer.write(" />");
    ...
    protected static void writeProcessorState(PrintWriter writer, ObjectName pName, MBeanServer mBeanServer, int mode) throws Exception {
        ...
        if (mode == 0) {
            ...
            if (fullStatus) {
                ...
                if (showRequest) {
                    writer.write(Escape.htmlElementContext(mBeanServer.getAttribute(pName, JamXmlElements.METHOD)));
                    writer.write(32);
        ...
        if (mode == 1) {
            writer.write("<worker ");
            writer.write(" stage=\"" + stageStr + SymbolConstants.QUOTES_SYMBOL);
            if (fullStatus) {
                writer.write(" requestProcessingTime=\"" + mBeanServer.getAttribute(pName, "requestProcessingTime") + SymbolConstants.QUOTES_SYMBOL);
                ...
                writer.write(SymbolConstants.QUOTES_SYMBOL);
                writer.write(" remoteAddr=\"" + Escape.htmlElementContext(mBeanServer.getAttribute(pName, "remoteAddr")) + SymbolConstants.QUOTES_SYMBOL);
                writer.write(" virtualHost=\"" + Escape.htmlElementContext(mBeanServer.getAttribute(pName, "virtualHost")) + SymbolConstants.QUOTES_SYMBOL);
                if (showRequest) {
                    writer.write(" method=\"" + Escape.htmlElementContext(mBeanServer.getAttribute(pName, JamXmlElements.METHOD)) + SymbolConstants.QUOTES_SYMBOL);
                    writer.write(" currentUri=\"" + Escape.htmlElementContext(mBeanServer.getAttribute(pName, "currentUri")) + SymbolConstants.QUOTES_SYMBOL);
                    String queryString2 = (String) mBeanServer.getAttribute(pName, "currentQueryString");
                    if (queryString2 != null && !queryString2.equals("")) {
                        writer.write(" currentQueryString=\"" + Escape.htmlElementContent(queryString2) + SymbolConstants.QUOTES_SYMBOL);
                    } else {
                        writer.write(" currentQueryString=\"&#63;\"");
                    }
                    writer.write(" protocol=\"" + Escape.htmlElementContext(mBeanServer.getAttribute(pName, "protocol")) + SymbolConstants.QUOTES_SYMBOL);
                } else {
                    writer.write(" method=\"&#63;\"");
    ...
    protected static void writeContext(PrintWriter writer, ObjectName objectName, MBeanServer mBeanServer, int mode) throws Exception {
        if (mode == 0) {
            ...
            if (slash != -1) {
                ...
                writer.print("</a>");
                writer.print("<p>");
                Object startTime = mBeanServer.getAttribute(objectName, AuthConstants.AUTH_PARAM_START_TIME_KEY);
    ...
    public static String formatSize(Object obj, boolean mb) {
        ...
        if (mb) {
            ...
            long mbytes = bytes / 1048576;
            long rest = ((bytes - (mbytes * 1048576)) * 100) / 1048576;
