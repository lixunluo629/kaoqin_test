...
/* loaded from: commons-codec-1.10.jar:org/apache/commons/codec/digest/HmacUtils.class */
public final class HmacUtils {
    ...
    public static Mac updateHmac(Mac mac, byte[] valueToDigest) throws IllegalStateException {
        mac.reset();
        mac.update(valueToDigest);
        return mac;
    }

    public static Mac updateHmac(Mac mac, InputStream valueToDigest) throws IllegalStateException, IOException {
        mac.reset();
        byte[] buffer = new byte[1024];
        int i = valueToDigest.read(buffer, 0, 1024);
        while (true) {
            int read = i;
            if (read > -1) {
                mac.update(buffer, 0, read);
                i = valueToDigest.read(buffer, 0, 1024);
            } else {
                return mac;
            }
        }
    ...
    public static Mac updateHmac(Mac mac, String valueToDigest) throws IllegalStateException {
        mac.reset();
