...
/* loaded from: commons-httpclient-3.1.jar:org/apache/commons/httpclient/auth/NTLM.class */
final class NTLM {
    ...
    private Cipher getCipher(byte[] key) throws NoSuchPaddingException, NoSuchAlgorithmException, AuthenticationException, InvalidKeyException {
        try {
    ...
    private byte[] setupKey(byte[] key56) {
        byte[] key = new byte[8];
        key[0] = (byte) ((key56[0] >> 1) & 255);
        key[1] = (byte) ((((key56[0] & 1) << 6) | (((key56[1] & 255) >> 2) & 255)) & 255);
        key[2] = (byte) ((((key56[1] & 3) << 5) | (((key56[2] & 255) >> 3) & 255)) & 255);
        key[3] = (byte) ((((key56[2] & 7) << 4) | (((key56[3] & 255) >> 4) & 255)) & 255);
        key[4] = (byte) ((((key56[3] & 15) << 3) | (((key56[4] & 255) >> 5) & 255)) & 255);
        key[5] = (byte) ((((key56[4] & 31) << 2) | (((key56[5] & 255) >> 6) & 255)) & 255);
        key[6] = (byte) ((((key56[5] & 63) << 1) | (((key56[6] & 255) >> 7) & 255)) & 255);
    ...
    private byte[] encrypt(byte[] key, byte[] bytes) throws BadPaddingException, NoSuchPaddingException, IllegalBlockSizeException, NoSuchAlgorithmException, AuthenticationException, InvalidKeyException {
    ...
    public String getType3Message(String user, String password, String host, String domain, byte[] nonce) throws AuthenticationException {
        ...
        int finalLength = 64 + 0 + 24 + domainLen + userLen + hostLen;
        prepareResponse(finalLength);
    ...
    private byte[] hashPassword(String password, byte[] nonce) throws BadPaddingException, NoSuchPaddingException, IllegalBlockSizeException, NoSuchAlgorithmException, AuthenticationException, InvalidKeyException {
    ...
    private void calcResp(byte[] keys, byte[] plaintext, byte[] results) throws BadPaddingException, NoSuchPaddingException, IllegalBlockSizeException, NoSuchAlgorithmException, AuthenticationException, InvalidKeyException {
    ...
    private byte[] convertShort(int num) {
        ...
        while (true) {
            String hex = string;
            if (hex.length() < 4) {
                string = new StringBuffer().append("0").append(hex).toString();
            } else {
