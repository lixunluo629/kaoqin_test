...
/* loaded from: mybatis-3.4.6.jar:org/apache/ibatis/ognl/Evaluation.class */
public class Evaluation {
    ...
    public String toString(boolean compact, boolean showChildren, String depth) {
        String stringResult;
        if (compact) {
            stringResult = depth + "<" + this.node.getClass().getName() + SymbolConstants.SPACE_SYMBOL + System.identityHashCode(this) + ">";
        } else {
            String ss = this.source != null ? this.source.getClass().getName() : "null";
            String rs = this.result != null ? this.result.getClass().getName() : "null";
            stringResult = depth + "<" + this.node.getClass().getName() + ": [" + (this.setOperation ? "set" : BeanUtil.PREFIX_GETTER_GET) + "] source = " + ss + ", result = " + this.result + " [" + rs + "]>";
        }
        if (showChildren) {
            stringResult = stringResult + ScriptUtils.FALLBACK_STATEMENT_SEPARATOR;
            for (Evaluation child = this.firstChild; child != null; child = child.next) {
