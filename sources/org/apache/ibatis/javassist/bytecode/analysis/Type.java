...
/* loaded from: mybatis-3.4.6.jar:org/apache/ibatis/javassist/bytecode/analysis/Type.class */
public class Type {
    ...
    Map findCommonInterfaces(Map typeMap, Map alterMap) {
        ...
        Iterator i2 = new ArrayList(alterMap.values()).iterator();
        while (i2.hasNext()) {
            CtClass intf = (CtClass) i2.next();
            try {
                CtClass[] interfaces = intf.getInterfaces();
                for (CtClass ctClass : interfaces) {
                    alterMap.remove(ctClass.getName());
                }
            } catch (NotFoundException e) {
                throw new RuntimeException(e);
            }
