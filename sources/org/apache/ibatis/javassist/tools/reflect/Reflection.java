...
/* loaded from: mybatis-3.4.6.jar:org/apache/ibatis/javassist/tools/reflect/Reflection.class */
public class Reflection implements Translator {
    ...
    static final String metaobjectClassName = "org.apache.ibatis.javassist.tools.reflect.Metaobject";
    static final String classMetaobjectClassName = "org.apache.ibatis.javassist.tools.reflect.ClassMetaobject";
    ...
    public void start(ClassPool pool) throws NotFoundException {
        this.classPool = pool;
        try {
            CtClass c = this.classPool.get("org.apache.ibatis.javassist.tools.reflect.Sample");
            ...
            throw new RuntimeException("org.apache.ibatis.javassist.tools.reflect.Sample is not found or broken.");
    ...
    @Override // org.apache.ibatis.javassist.Translator
    public void onLoad(ClassPool pool, String classname) throws NotFoundException, RuntimeException, CannotCompileException {
        CtClass clazz = pool.get(classname);
        clazz.instrument(this.converter);
    }

    public boolean makeReflective(String classname, String metaobject, String metaclass) throws NotFoundException, CannotCompileException {
        return makeReflective(this.classPool.get(classname), this.classPool.get(metaobject), this.classPool.get(metaclass));
    }

    public boolean makeReflective(Class clazz, Class metaobject, Class metaclass) throws NotFoundException, CannotCompileException {
        return makeReflective(clazz.getName(), metaobject.getName(), metaclass.getName());
    }

    public boolean makeReflective(CtClass clazz, CtClass metaobject, CtClass metaclass) throws NotFoundException, CannotCompileException {
    ...
    private boolean modifyClassfile(CtClass clazz, CtClass metaobject, CtClass metaclass) throws NotFoundException, RuntimeException, CannotCompileException {
        if (clazz.getAttribute("Reflective") != null) {
            return false;
        }
        clazz.setAttribute("Reflective", new byte[0]);
        CtClass mlevel = this.classPool.get("org.apache.ibatis.javassist.tools.reflect.Metalevel");
    ...
    private void processMethods(CtClass clazz, boolean dontSearch) throws NotFoundException, RuntimeException, CannotCompileException {
    ...
    private void processMethods0(int mod, CtClass clazz, CtMethod m, int identifier, boolean dontSearch) throws NotFoundException, RuntimeException, CannotCompileException {
        CtMethod m2;
        CtMethod body;
    ...
    private void processFields(CtClass clazz) throws NotFoundException, RuntimeException, CannotCompileException {
        CtField[] fs = clazz.getDeclaredFields();
        for (CtField f : fs) {
            int mod = f.getModifiers();
            if ((mod & 1) != 0 && (mod & 16) == 0) {
