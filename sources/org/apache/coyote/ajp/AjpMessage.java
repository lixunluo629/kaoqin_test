...
/* loaded from: tomcat-embed-core-8.5.43.jar:org/apache/coyote/ajp/AjpMessage.class */
public class AjpMessage {
    ...
    public int processHeader(boolean toContainer) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Received " + this.len + SymbolConstants.SPACE_SYMBOL + ((int) this.buf[0]));
        }
        return this.len;
    }
    ...
    private void dump(String prefix) {
        if (log.isDebugEnabled()) {
            log.debug(prefix + ": " + HexUtils.toHexString(this.buf) + SymbolConstants.SPACE_SYMBOL + this.pos + "/" + (this.len + 4));
        }
        int max = this.pos;
    ...
    protected static String hexLine(byte[] buf, int start, int len) {
        ...
        for (int i = start; i < start + 16; i++) {
            if (i < len + 4) {
                sb.append(hex(buf[i]) + SymbolConstants.SPACE_SYMBOL);
            } else {
                sb.append("   ");
            }
        }
