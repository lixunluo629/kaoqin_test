...
/* loaded from: jboss-logging-3.3.2.Final.jar:org/jboss/logging/Logger.class */
public abstract class Logger implements Serializable, BasicLogger {
    ...
    /* loaded from: jboss-logging-3.3.2.Final.jar:org/jboss/logging/Logger$Level.class */
    public enum Level {
        FATAL,
        ERROR,
        WARN,
        ...
        return (T) AccessController.doPrivileged(new PrivilegedAction<T>() { // from class: org.jboss.logging.Logger.1
            ...
            public T run() {
                ...
                if (variant != null && variant.length() > 0) {
                    try {
                        clsAsSubclass = Class.forName(Logger.join(typeName, "$logger", language, country, variant), true, classLoader).asSubclass(cls);
                    } catch (ClassNotFoundException e) {
                    }
                }
                if (clsAsSubclass == null && country != null && country.length() > 0) {
                    try {
                        clsAsSubclass = Class.forName(Logger.join(typeName, "$logger", language, country, null), true, classLoader).asSubclass(cls);
                    } catch (ClassNotFoundException e2) {
                    }
                }
                if (clsAsSubclass == null && language != null && language.length() > 0) {
                    try {
                        clsAsSubclass = Class.forName(Logger.join(typeName, "$logger", language, null, null), true, classLoader).asSubclass(cls);
                    } catch (ClassNotFoundException e3) {
                    }
                }
                if (clsAsSubclass == null) {
                    try {
                        clsAsSubclass = Class.forName(Logger.join(typeName, "$logger", null, null, null), true, classLoader).asSubclass(cls);
                ...
                try {
                    try {
                        return clsAsSubclass.getConstructor(Logger.class).newInstance(Logger.getLogger(str));
                    } catch (IllegalAccessException e5) {
                        throw new IllegalArgumentException("Logger implementation " + clsAsSubclass + " could not be instantiated", e5);
                    } catch (InstantiationException e6) {
                        throw new IllegalArgumentException("Logger implementation " + clsAsSubclass + " could not be instantiated", e6);
                    } catch (InvocationTargetException e7) {
                        throw new IllegalArgumentException("Logger implementation " + clsAsSubclass + " could not be instantiated", e7.getCause());
                    }
                } catch (NoSuchMethodException e8) {
                    throw new IllegalArgumentException("Logger implementation " + clsAsSubclass + " has no matching constructor");
                }
            }
        });
    }
