...
/* loaded from: bcprov-jdk15on-1.64.jar:org/bouncycastle/pqc/jcajce/provider/mceliece/McEliecePKCSCipherSpi.class */
public class McEliecePKCSCipherSpi extends AsymmetricBlockCipher implements PKCSObjectIdentifiers, X509ObjectIdentifiers {
    ...
    /* loaded from: bcprov-jdk15on-1.64.jar:org/bouncycastle/pqc/jcajce/provider/mceliece/McEliecePKCSCipherSpi$McEliecePKCS.class */
    ...
    protected byte[] messageEncrypt(byte[] bArr) throws BadPaddingException, IllegalBlockSizeException {
        byte[] bArrMessageEncrypt = null;
        try {
            bArrMessageEncrypt = this.cipher.messageEncrypt(bArr);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return bArrMessageEncrypt;
    }

    @Override // org.bouncycastle.pqc.jcajce.provider.util.AsymmetricBlockCipher
    protected byte[] messageDecrypt(byte[] bArr) throws BadPaddingException, IllegalBlockSizeException {
        byte[] bArrMessageDecrypt = null;
        try {
            bArrMessageDecrypt = this.cipher.messageDecrypt(bArr);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return bArrMessageDecrypt;
    }
