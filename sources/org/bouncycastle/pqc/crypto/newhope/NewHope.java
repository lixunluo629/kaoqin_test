...
/* loaded from: bcprov-jdk15on-1.64.jar:org/bouncycastle/pqc/crypto/newhope/NewHope.class */
class NewHope {
    ...
    static void decodeA(short[] sArr, byte[] bArr, byte[] bArr2) {
        ...
        System.arraycopy(bArr2, MysqlErrorNumbers.ER_CANT_EXECUTE_IN_READ_ONLY_TRANSACTION, bArr, 0, 32);
    }
    ...
    static void decodeB(short[] sArr, short[] sArr2, byte[] bArr) {
        ...
        for (int i = 0; i < 256; i++) {
            int i2 = 4 * i;
            int i3 = bArr[MysqlErrorNumbers.ER_CANT_EXECUTE_IN_READ_ONLY_TRANSACTION + i] & 255;
    ...
    static void encodeA(byte[] bArr, short[] sArr, byte[] bArr2) {
        ...
        System.arraycopy(bArr2, 0, bArr, MysqlErrorNumbers.ER_CANT_EXECUTE_IN_READ_ONLY_TRANSACTION, 32);
    }
    ...
    static void encodeB(byte[] bArr, short[] sArr, short[] sArr2) {
        ...
        for (int i = 0; i < 256; i++) {
            int i2 = 4 * i;
            bArr[MysqlErrorNumbers.ER_CANT_EXECUTE_IN_READ_ONLY_TRANSACTION + i] = (byte) (sArr2[i2] | (sArr2[i2 + 1] << 2) | (sArr2[i2 + 2] << 4) | (sArr2[i2 + 3] << 6));
        }
    }
