...
/* loaded from: bcprov-jdk15on-1.64.jar:org/bouncycastle/pqc/crypto/sphincs/SPHINCS256Signer.class */
public class SPHINCS256Signer implements MessageSigner {
    ...
    byte[] crypto_sign(HashFunctions hashFunctions, byte[] bArr, byte[] bArr2) throws IllegalArgumentException {
        ...
        System.arraycopy(bArr9, MysqlErrorNumbers.ER_WRONG_GROUP_FIELD, bArr3, Constants.SC_RESP_SET_COOKIE2, 32);
        Digest messageHash = hashFunctions.getMessageHash();
        byte[] bArr10 = new byte[messageHash.getDigestSize()];
        messageHash.update(bArr3, Constants.SC_RESP_SET_COOKIE2, 32);
        messageHash.update(bArr, 0, bArr.length);
        messageHash.doFinal(bArr10, 0);
        zerobytes(bArr3, Constants.SC_RESP_SET_COOKIE2, 32);
        ...
        new Horst();
        int iHorst_sign = i6 + Horst.horst_sign(hashFunctions, bArr3, i6, bArr6, bArr7, bArr8, bArr5);
        Wots wots = new Wots();
        for (int i7 = 0; i7 < 12; i7++) {
            leafaddrVar2.level = i7;
            Seed.get_seed(hashFunctions, bArr7, 0, bArr9, leafaddrVar2);
            wots.wots_sign(hashFunctions, bArr3, iHorst_sign, bArr6, bArr7, bArr8);
            int i8 = iHorst_sign + 2144;
    ...
    boolean verify(HashFunctions hashFunctions, byte[] bArr, byte[] bArr2, byte[] bArr3) {
        ...
        byte[] bArr8 = new byte[MysqlErrorNumbers.ER_WRONG_GROUP_FIELD];
        if (length != 41000) {
        ...
        messageHash.update(bArr8, 0, MysqlErrorNumbers.ER_WRONG_GROUP_FIELD);
        messageHash.update(bArr, 0, bArr.length);
