...
/* loaded from: bcprov-jdk15on-1.64.jar:org/bouncycastle/crypto/kems/ECIESKeyEncapsulation.class */
public class ECIESKeyEncapsulation implements KeyEncapsulation {
    ...
    protected KeyParameter deriveKey(int i, byte[] bArr, byte[] bArr2) {
        byte[] bArrConcatenate = bArr2;
        if (!this.SingleHashMode) {
            bArrConcatenate = Arrays.concatenate(bArr, bArr2);
            Arrays.fill(bArr2, (byte) 0);
        }
        try {
            this.kdf.init(new KDFParameters(bArrConcatenate, null));
            byte[] bArr3 = new byte[i];
            this.kdf.generateBytes(bArr3, 0, bArr3.length);
            KeyParameter keyParameter = new KeyParameter(bArr3);
            Arrays.fill(bArrConcatenate, (byte) 0);
            return keyParameter;
        } catch (Throwable th) {
            Arrays.fill(bArrConcatenate, (byte) 0);
            throw th;
