...
/* loaded from: bcprov-jdk15on-1.64.jar:org/bouncycastle/crypto/prng/drbg/HMacSP800DRBG.class */
public class HMacSP800DRBG implements SP80090DRBG {
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) throws IllegalStateException, DataLengthException, IllegalArgumentException {
        ...
        for (int i = 0; i < length2; i++) {
            this._hMac.update(this._V, 0, this._V.length);
            this._hMac.doFinal(this._V, 0);
            System.arraycopy(this._V, 0, bArr3, i * this._V.length, this._V.length);
        }
        if (length2 * this._V.length < bArr3.length) {
            this._hMac.update(this._V, 0, this._V.length);
            this._hMac.doFinal(this._V, 0);
            System.arraycopy(this._V, 0, bArr3, length2 * this._V.length, bArr3.length - (length2 * this._V.length));
        }
        hmac_DRBG_Update(bArr2);
        this._reseedCounter++;
