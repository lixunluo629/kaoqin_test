...
/* loaded from: bcpkix-jdk15on-1.64.jar:org/bouncycastle/cms/CMSSignedDataParser.class */
public class CMSSignedDataParser extends CMSContentInfoParser {
    ...
    public static OutputStream replaceSigners(InputStream inputStream, SignerInformationStore signerInformationStore, OutputStream outputStream) throws CMSException, IOException {
        ...
        while (it.hasNext()) {
            aSN1EncodableVector.add((ASN1Encodable) CMSSignedHelper.INSTANCE.fixAlgID(it.next().getDigestAlgorithmID()));
        }
        ...
        while (it2.hasNext()) {
            aSN1EncodableVector2.add((ASN1Encodable) it2.next().toASN1Structure());
        }
    ...
    private static void writeSetToGeneratorTagged(ASN1Generator aSN1Generator, ASN1SetParser aSN1SetParser, int i) throws IOException {
        ...
        if (aSN1Set != null) {
            if (aSN1SetParser instanceof BERSetParser) {
                aSN1Generator.getRawOutputStream().write(new BERTaggedObject(false, i, (ASN1Encodable) aSN1Set).getEncoded());
            } else {
                aSN1Generator.getRawOutputStream().write(new DERTaggedObject(false, i, (ASN1Encodable) aSN1Set).getEncoded());
            }
        }
    }
