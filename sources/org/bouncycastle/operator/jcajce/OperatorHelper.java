...
/* loaded from: bcpkix-jdk15on-1.64.jar:org/bouncycastle/operator/jcajce/OperatorHelper.class */
class OperatorHelper {
    ...
    /* loaded from: bcpkix-jdk15on-1.64.jar:org/bouncycastle/operator/jcajce/OperatorHelper$OpCertificateException.class */
    ...
    MessageDigest createDigest(AlgorithmIdentifier algorithmIdentifier) throws GeneralSecurityException {
        ...
        try {
            messageDigestCreateMessageDigest = this.helper.createMessageDigest(MessageDigestUtils.getDigestName(algorithmIdentifier.getAlgorithm()));
        } catch (NoSuchAlgorithmException e) {
            if (oids.get(algorithmIdentifier.getAlgorithm()) == null) {
                throw e;
            }
            messageDigestCreateMessageDigest = this.helper.createMessageDigest((String) oids.get(algorithmIdentifier.getAlgorithm()));
        }
    ...
    Signature createSignature(AlgorithmIdentifier algorithmIdentifier) throws GeneralSecurityException, IOException {
        ...
        try {
            signatureCreateSignature = this.helper.createSignature(getSignatureName(algorithmIdentifier));
        } catch (NoSuchAlgorithmException e) {
            if (oids.get(algorithmIdentifier.getAlgorithm()) == null) {
                throw e;
            }
            signatureCreateSignature = this.helper.createSignature((String) oids.get(algorithmIdentifier.getAlgorithm()));
        }
    ...
    public Signature createRawSignature(AlgorithmIdentifier algorithmIdentifier) throws InvalidAlgorithmParameterException {
        try {
            ...
            String str = Constants.NONE_TRANSPORT + signatureName.substring(signatureName.indexOf("WITH"));
    ...
    static {
        ...
        symmetricKeyAlgNames.put(NISTObjectIdentifiers.aes, RSAUtils.AES_KEY_ALGORITHM);
        symmetricKeyAlgNames.put(NISTObjectIdentifiers.id_aes128_CBC, RSAUtils.AES_KEY_ALGORITHM);
        symmetricKeyAlgNames.put(NISTObjectIdentifiers.id_aes192_CBC, RSAUtils.AES_KEY_ALGORITHM);
        symmetricKeyAlgNames.put(NISTObjectIdentifiers.id_aes256_CBC, RSAUtils.AES_KEY_ALGORITHM);
