...
/* loaded from: spring-web-4.3.25.RELEASE.jar:org/springframework/http/converter/json/Jackson2ObjectMapperBuilder.class */
public class Jackson2ObjectMapperBuilder {
    ...
    public <T extends ObjectMapper> T build() {
        ...
        if (this.createXmlMapper) {
            if (this.defaultUseWrapper != null) {
                objectMapperCreate = new XmlObjectMapperInitializer().create(this.defaultUseWrapper.booleanValue());
            } else {
                objectMapperCreate = new XmlObjectMapperInitializer().create();
            }
            objectMapper = objectMapperCreate;
        } else {
    ...
    public void configure(ObjectMapper objectMapper) {
        ...
        if (this.findModulesViaServiceLoader) {
            Iterator<Module> it = ObjectMapper.findModules(this.moduleClassLoader).iterator();
            while (it.hasNext()) {
                registerModule(it.next(), modulesToRegister);
            }
        ...
        if (this.modules != null) {
            ...
            while (it2.hasNext()) {
                registerModule(it2.next(), modulesToRegister);
            }
        }
        ...
        Iterator<Module> it3 = modulesToRegister.values().iterator();
        while (it3.hasNext()) {
            List<Module> nestedModules = (List) it3.next();
            Iterator<Module> it4 = nestedModules.iterator();
            while (it4.hasNext()) {
                objectMapper.registerModule(it4.next());
            }
        }
        ...
        for (Object feature : this.features.keySet()) {
            configureFeature(objectMapper, feature, this.features.get(feature).booleanValue());
        }
    ...
    private <T> void addSerializers(SimpleModule module) {
        Iterator<Class<?>> it = this.serializers.keySet().iterator();
        while (it.hasNext()) {
            Class<? extends T> cls = (Class) it.next();
            module.addSerializer(cls, (JsonSerializer) this.serializers.get(cls));
        }
    }

    private <T> void addDeserializers(SimpleModule module) {
        Iterator<Class<?>> it = this.deserializers.keySet().iterator();
        while (it.hasNext()) {
            Class<T> cls = (Class) it.next();
            module.addDeserializer(cls, (JsonDeserializer) this.deserializers.get(cls));
        }
    }
    ...
    private void registerWellKnownModulesIfAvailable(MultiValueMap<Object, Module> modulesToRegister) {
        if (ClassUtils.isPresent("java.nio.file.Path", this.moduleClassLoader)) {
            try {
                Module jdk7Module = (Module) BeanUtils.instantiateClass(ClassUtils.forName("com.fasterxml.jackson.datatype.jdk7.Jdk7Module", this.moduleClassLoader));
    ...
    /* loaded from: spring-web-4.3.25.RELEASE.jar:org/springframework/http/converter/json/Jackson2ObjectMapperBuilder$XmlObjectMapperInitializer.class */
    private static class XmlObjectMapperInitializer {
        ...
        private static XMLInputFactory xmlInputFactory() throws IllegalArgumentException, FactoryConfigurationError {
