...
/* loaded from: spring-expression-4.3.25.RELEASE.jar:org/springframework/expression/spel/CodeFlow.class */
public class CodeFlow implements Opcodes {
    ...
    @FunctionalInterface
    /* loaded from: spring-expression-4.3.25.RELEASE.jar:org/springframework/expression/spel/CodeFlow$ClinitAdder.class */
    public interface ClinitAdder {
        void generateCode(MethodVisitor methodVisitor, CodeFlow codeFlow);
    }

    @FunctionalInterface
    /* loaded from: spring-expression-4.3.25.RELEASE.jar:org/springframework/expression/spel/CodeFlow$FieldAdder.class */
    ...
    public static void insertUnboxInsns(MethodVisitor mv, char ch2, String stackDescriptor) {
        switch (ch2) {
            case 'B':
        ...
        if (descriptor.length() != 1) {
            if (descriptor.charAt(0) == '[') {
                if (isPrimitiveArray(descriptor)) {
                    ...
                    mv.visitTypeInsn(192, descriptor + ScriptUtils.DEFAULT_STATEMENT_SEPARATOR);
                    return;
                }
            }
    ...
    public static String toDescriptor(Class<?> type) {
        ...
        if (name.charAt(0) != '[') {
            return StandardRoles.L + type.getName().replace('.', '/');
        }
        if (name.endsWith(ScriptUtils.DEFAULT_STATEMENT_SEPARATOR)) {
    ...
    public static void insertNewArrayCode(MethodVisitor mv, int size, String arraytype) {
        ...
        if (arraytype.charAt(0) == '[') {
            if (isReferenceTypeArray(arraytype)) {
                mv.visitTypeInsn(189, arraytype + ScriptUtils.DEFAULT_STATEMENT_SEPARATOR);
                return;
            } else {
