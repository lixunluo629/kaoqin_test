...
/* loaded from: spring-webmvc-4.3.25.RELEASE.jar:org/springframework/web/servlet/view/ContentNegotiatingViewResolver.class */
public class ContentNegotiatingViewResolver extends WebApplicationObjectSupport implements ViewResolver, Ordered, InitializingBean {
        ...
        if (this.viewResolvers == null) {
            ...
            for (int i = 0; i < this.viewResolvers.size(); i++) {
                if (!matchingBeans.contains(this.viewResolvers.get(i))) {
                    ...
                    getApplicationContext().getAutowireCapableBeanFactory().initializeBean(this.viewResolvers.get(i), name);
                }
            }
        }
    ...
    protected List<MediaType> getMediaTypes(HttpServletRequest request) {
        try {
            ...
            List<MediaType> acceptableMediaTypes2 = !acceptableMediaTypes.isEmpty() ? acceptableMediaTypes : Collections.singletonList(MediaType.ALL);
            List<MediaType> producibleMediaTypes = getProducibleMediaTypes(request);
            Set<MediaType> compatibleMediaTypes = new LinkedHashSet<>();
            for (MediaType acceptable : acceptableMediaTypes2) {
            ...
            for (View candidateView2 : candidateViews) {
                if (StringUtils.hasText(candidateView2.getContentType())) {
                    ...
                    if (mediaType.isCompatibleWith(candidateContentType)) {
                        if (this.logger.isDebugEnabled()) {
                            this.logger.debug("Returning [" + candidateView2 + "] based on requested media type '" + mediaType + "'");
                        }
