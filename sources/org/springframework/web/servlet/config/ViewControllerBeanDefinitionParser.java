...
/* loaded from: spring-webmvc-4.3.25.RELEASE.jar:org/springframework/web/servlet/config/ViewControllerBeanDefinitionParser.class */
class ViewControllerBeanDefinitionParser implements BeanDefinitionParser {
    ...
    public BeanDefinition parse(Element element, ParserContext parserContext) throws NumberFormatException, BeanDefinitionStoreException {
        ...
        if (hm.getPropertyValues().contains("urlMap")) {
            urlMap = (Map) hm.getPropertyValues().getPropertyValue("urlMap").getValue();
        } else {
            urlMap = new ManagedMap<>();
            hm.getPropertyValues().add("urlMap", urlMap);
        }
        urlMap.put(element.getAttribute(Cookie2.PATH), controller);
        return null;
    }

    private BeanDefinition registerHandlerMapping(ParserContext context, Object source) throws BeanDefinitionStoreException {
