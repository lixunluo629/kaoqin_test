...
/* loaded from: spring-tx-4.3.25.RELEASE.jar:org/springframework/jca/work/WorkManagerTaskExecutor.class */
        ...
        try {
            if (this.blockUntilCompleted) {
                if (startTimeout != Long.MAX_VALUE || this.workListener != null) {
                    this.workManager.doWork(work, startTimeout, (ExecutionContext) null, this.workListener);
                } else {
                    this.workManager.doWork(work);
                }
            } else if (this.blockUntilStarted) {
                if (startTimeout != Long.MAX_VALUE || this.workListener != null) {
                    this.workManager.startWork(work, startTimeout, (ExecutionContext) null, this.workListener);
                } else {
                    this.workManager.startWork(work);
                }
            } else if (startTimeout != Long.MAX_VALUE || this.workListener != null) {
                this.workManager.scheduleWork(work, startTimeout, (ExecutionContext) null, this.workListener);
            } else {
            ...
            throw new SchedulingException("Could not schedule task on JCA WorkManager", ex2);
        }
    }
