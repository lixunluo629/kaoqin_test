...
/* loaded from: spring-tx-4.3.25.RELEASE.jar:org/springframework/jca/work/SimpleTaskWorkManager.class */
public class SimpleTaskWorkManager implements WorkManager {
    ...
    public void doWork(Work work) throws WorkException, WorkRejectedException {
        doWork(work, Long.MAX_VALUE, null, null);
    }

    public void doWork(Work work, long startTimeout, ExecutionContext executionContext, WorkListener workListener) throws WorkException, WorkRejectedException {
    ...
    public void scheduleWork(Work work) throws WorkException, WorkRejectedException {
        scheduleWork(work, Long.MAX_VALUE, null, null);
    }

    public void scheduleWork(Work work, long startTimeout, ExecutionContext executionContext, WorkListener workListener) throws WorkException, WorkRejectedException {
        Assert.state(this.asyncTaskExecutor != null, "No 'asyncTaskExecutor' set");
        executeWork(this.asyncTaskExecutor, work, startTimeout, false, executionContext, workListener);
    }

    /* JADX INFO: Thrown type has an unknown type hierarchy: javax.resource.spi.work.WorkException */
    /* JADX INFO: Thrown type has an unknown type hierarchy: javax.resource.spi.work.WorkRejectedException */
    /* JADX WARN: Multi-variable type inference failed */
    /* JADX WARN: Type inference failed for: r0v5, types: [java.lang.Runnable, org.springframework.jca.work.SimpleTaskWorkManager$DelegatingWorkAdapter] */
    protected long executeWork(TaskExecutor taskExecutor, Work work, long startTimeout, boolean blockUntilStarted, ExecutionContext executionContext, WorkListener workListener) throws WorkException, WorkRejectedException {
        ...
        ?? delegatingWorkAdapter = new DelegatingWorkAdapter(work, workListenerToUse, !isAsync);
        try {
            if (isAsync) {
                ((AsyncTaskExecutor) taskExecutor).execute(delegatingWorkAdapter, startTimeout);
            } else {
            ...
            WorkRejectedException workRejectedException = new WorkRejectedException("TaskExecutor rejected Work because of timeout: " + work, ex);
        ...
        } catch (TaskRejectedException ex2) {
            WorkRejectedException workRejectedException2 = new WorkRejectedException("TaskExecutor rejected Work: " + work, ex2);
    ...
    /* loaded from: spring-tx-4.3.25.RELEASE.jar:org/springframework/jca/work/SimpleTaskWorkManager$DelegatingWorkAdapter.class */
