...
/* loaded from: spring-context-4.3.25.RELEASE.jar:org/springframework/scripting/support/ScriptFactoryPostProcessor.class */
public class ScriptFactoryPostProcessor extends InstantiationAwareBeanPostProcessorAdapter implements BeanClassLoaderAware, BeanFactoryAware, ResourceLoaderAware, DisposableBean, Ordered {
    ...
    public static final String LANGUAGE_ATTRIBUTE = Conventions.getQualifiedAttributeName(ScriptFactoryPostProcessor.class, PdfConst.Language);
    ...
    protected Class<?> createConfigInterface(BeanDefinition bd, Class<?>[] interfaces) throws BeansException {
        ...
        if (bd instanceof AbstractBeanDefinition) {
            ...
            if (abd.getInitMethodName() != null) {
                Signature signature2 = new Signature(abd.getInitMethodName(), Type.VOID_TYPE, new Type[0]);
                maker.add(signature2, new Type[0]);
            }
            if (StringUtils.hasText(abd.getDestroyMethodName())) {
                Signature signature3 = new Signature(abd.getDestroyMethodName(), Type.VOID_TYPE, new Type[0]);
