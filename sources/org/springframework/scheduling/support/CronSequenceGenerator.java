...
/* loaded from: spring-context-4.3.25.RELEASE.jar:org/springframework/scheduling/support/CronSequenceGenerator.class */
public class CronSequenceGenerator {
    ...
    private int findNextDay(Calendar calendar, BitSet daysOfMonth, int dayOfMonth, BitSet daysOfWeek, int dayOfWeek, List<Integer> resets) {
        ...
        if (count >= 366) {
            throw new IllegalArgumentException("Overflow in day for expression \"" + this.expression + SymbolConstants.QUOTES_SYMBOL);
        }
        return dayOfMonth;
    }
    ...
    private void parse(String expression) throws IllegalArgumentException {
        String[] fields = StringUtils.tokenizeToStringArray(expression, SymbolConstants.SPACE_SYMBOL);
    ...
    private void setNumberHits(BitSet bits, String value, int min, int max) throws NumberFormatException {
        ...
        for (String field : fields) {
            if (!field.contains("/")) {
                ...
                if (split.length > 2) {
                    throw new IllegalArgumentException("Incrementer has more than two fields: '" + field + "' in expression \"" + this.expression + SymbolConstants.QUOTES_SYMBOL);
                }
                ...
                if (delta <= 0) {
                    throw new IllegalArgumentException("Incrementer delta must be 1 or higher: '" + field + "' in expression \"" + this.expression + SymbolConstants.QUOTES_SYMBOL);
                }
                int i = range2[0];
                while (true) {
                    int i2 = i;
    ...
    private int[] getRange(String field, int min, int max) {
        ...
        if (!field.contains("-")) {
            ...
            String[] split = StringUtils.delimitedListToStringArray(field, "-");
            if (split.length > 2) {
                throw new IllegalArgumentException("Range has more than two fields: '" + field + "' in expression \"" + this.expression + SymbolConstants.QUOTES_SYMBOL);
            }
            result[0] = Integer.valueOf(split[0]).intValue();
            result[1] = Integer.valueOf(split[1]).intValue();
        }
        if (result[0] >= max || result[1] >= max) {
            throw new IllegalArgumentException("Range exceeds maximum (" + max + "): '" + field + "' in expression \"" + this.expression + SymbolConstants.QUOTES_SYMBOL);
        }
        if (result[0] < min || result[1] < min) {
            throw new IllegalArgumentException("Range less than minimum (" + min + "): '" + field + "' in expression \"" + this.expression + SymbolConstants.QUOTES_SYMBOL);
        }
        if (result[0] > result[1]) {
            throw new IllegalArgumentException("Invalid inverted range: '" + field + "' in expression \"" + this.expression + SymbolConstants.QUOTES_SYMBOL);
        }
        return result;
    }
