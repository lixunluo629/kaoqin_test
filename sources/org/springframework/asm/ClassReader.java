...
/* loaded from: spring-core-4.3.25.RELEASE.jar:org/springframework/asm/ClassReader.class */
    ...
    */
    private static byte[] readClass(java.io.InputStream r6, boolean r7) throws java.io.IOException {
        ...
        L9f:
            ...
            throw r0
        */
        throw new UnsupportedOperationException("Method not decompiled: org.springframework.asm.ClassReader.readClass(java.io.InputStream, boolean):byte[]");
    }
    ...
    public void accept(ClassVisitor classVisitor, Attribute[] attrs, int flags) {
        ...
        for (int i2 = readUnsignedShort(u3); i2 > 0; i2--) {
            ...
            if (SourceFileAttribute.tag.equals(attrName)) {
                ...
                moduleMainClass = readClass(u3 + 8, c);
    ...
    private void readModule(ClassVisitor classVisitor, Context context, int u, String mainClass, int packages) {
        ...
        int u3 = u2 + 2;
        for (int i2 = readUnsignedShort(u3 - 2); i2 > 0; i2--) {
            String module = readModule(u3, buffer);
            int access = readUnsignedShort(u3 + 2);
            String requireVersion = readUTF8(u3 + 4, buffer);
            mv.visitRequire(module, access, requireVersion);
            u3 += 6;
        }
        int u4 = u3 + 2;
        for (int i3 = readUnsignedShort(u4 - 2); i3 > 0; i3--) {
            String export = readPackage(u4, buffer);
            int access2 = readUnsignedShort(u4 + 2);
            int exportToCount = readUnsignedShort(u4 + 4);
            u4 += 6;
            String[] tos = null;
            if (exportToCount != 0) {
                tos = new String[exportToCount];
                for (int j = 0; j < tos.length; j++) {
                    tos[j] = readModule(u4, buffer);
                    u4 += 2;
                }
            }
            mv.visitExport(export, access2, tos);
        }
        int u5 = u4 + 2;
        for (int i4 = readUnsignedShort(u5 - 2); i4 > 0; i4--) {
            String open = readPackage(u5, buffer);
            int access3 = readUnsignedShort(u5 + 2);
            int openToCount = readUnsignedShort(u5 + 4);
            u5 += 6;
            String[] tos2 = null;
            if (openToCount != 0) {
                tos2 = new String[openToCount];
                for (int j2 = 0; j2 < tos2.length; j2++) {
                    tos2[j2] = readModule(u5, buffer);
                    u5 += 2;
                }
            }
            mv.visitOpen(open, access3, tos2);
        }
        int u6 = u5 + 2;
        for (int i5 = readUnsignedShort(u6 - 2); i5 > 0; i5--) {
            mv.visitUse(readClass(u6, buffer));
            u6 += 2;
        }
        int u7 = u6 + 2;
        for (int i6 = readUnsignedShort(u7 - 2); i6 > 0; i6--) {
            String service = readClass(u7, buffer);
            int provideWithCount = readUnsignedShort(u7 + 2);
            u7 += 4;
            String[] withs = new String[provideWithCount];
            for (int j3 = 0; j3 < withs.length; j3++) {
                withs[j3] = readClass(u7, buffer);
                u7 += 2;
            }
    ...
    private int readMethod(ClassVisitor classVisitor, Context context, int u) {
        ...
        if (methodParameters != 0) {
            int i2 = this.b[methodParameters] & 255;
            ...
            switch (ClassWriter.TYPE[opcode]) {
                ...
                case 18:
                    ...
                    if (opcode5 == 167 || opcode5 == 168) {
                        ...
                        Label endif = readLabel(offset2 + 3, labels);
                    ...
                    break;
                case 19:
                    mv.visitJumpInsn(200, labels[offset2 + readInt(u6 + 1)]);
                    insertFrame = true;
                    u6 += 5;
                    break;
            }
    ...
    private int[] readTypeAnnotations(MethodVisitor mv, Context context, int u, boolean visible) {
        ...
        for (int i = 0; i < offsets.length; i++) {
            ...
            if ((target >>> 24) == 66) {
                ...
                annotationValues = readAnnotationValues(u4 + 2, c, true, mv.visitTryCatchAnnotation(target, path, readUTF8(u4, c), visible));
            } else {
                annotationValues = readAnnotationValues(u2 + 3 + (2 * pathLength), c, true, null);
            }
            u3 = annotationValues;
        }
        return offsets;
    ...
    private int readAnnotationTarget(Context context, int u) {
        ...
        switch (target2 >>> 24) {
            ...
            case 1:
            case 22:
                target = target2 & org.objectweb.asm.Opcodes.V_PREVIEW_EXPERIMENTAL;
                u2 = u + 2;
                break;
            case 19:
            case 20:
