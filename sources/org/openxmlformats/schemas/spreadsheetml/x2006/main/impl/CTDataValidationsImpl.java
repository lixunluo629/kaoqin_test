...
/* loaded from: poi-ooxml-schemas-3.17.jar:org/openxmlformats/schemas/spreadsheetml/x2006/main/impl/CTDataValidationsImpl.class */
public class CTDataValidationsImpl extends XmlComplexContentImpl implements CTDataValidations {
    private static final QName DATAVALIDATION$0 = new QName(XSSFRelation.NS_SPREADSHEETML, "dataValidation");
    ...
    public int sizeOfDataValidationArray() {
        ...
        synchronized (monitor()) {
            check_orphaned();
            iCount_elements = get_store().count_elements(DATAVALIDATION$0);
        }
    ...
    public boolean getDisablePrompts() {
        synchronized (monitor()) {
            check_orphaned();
            SimpleValue simpleValue = (SimpleValue) get_store().find_attribute_user(DISABLEPROMPTS$2);
            if (simpleValue == null) {
    ...
    public XmlBoolean xgetDisablePrompts() {
        ...
        synchronized (monitor()) {
            check_orphaned();
            XmlBoolean xmlBoolean2 = (XmlBoolean) get_store().find_attribute_user(DISABLEPROMPTS$2);
            if (xmlBoolean2 == null) {
                xmlBoolean2 = (XmlBoolean) get_default_attribute_value(DISABLEPROMPTS$2);
            }
    ...
    public void xsetDisablePrompts(XmlBoolean xmlBoolean) {
        synchronized (monitor()) {
            ...
            XmlBoolean xmlBoolean2 = (XmlBoolean) get_store().find_attribute_user(DISABLEPROMPTS$2);
            if (xmlBoolean2 == null) {
                xmlBoolean2 = (XmlBoolean) get_store().add_attribute_user(DISABLEPROMPTS$2);
            }
    ...
    public void xsetXWindow(XmlUnsignedInt xmlUnsignedInt) {
        synchronized (monitor()) {
            check_orphaned();
            XmlUnsignedInt xmlUnsignedInt2 = (XmlUnsignedInt) get_store().find_attribute_user(XWINDOW$4);
            if (xmlUnsignedInt2 == null) {
                xmlUnsignedInt2 = (XmlUnsignedInt) get_store().add_attribute_user(XWINDOW$4);
            }
    ...
    public void xsetYWindow(XmlUnsignedInt xmlUnsignedInt) {
        synchronized (monitor()) {
            check_orphaned();
            XmlUnsignedInt xmlUnsignedInt2 = (XmlUnsignedInt) get_store().find_attribute_user(YWINDOW$6);
            if (xmlUnsignedInt2 == null) {
                xmlUnsignedInt2 = (XmlUnsignedInt) get_store().add_attribute_user(YWINDOW$6);
            }
            xmlUnsignedInt2.set(xmlUnsignedInt);
        }
    }
