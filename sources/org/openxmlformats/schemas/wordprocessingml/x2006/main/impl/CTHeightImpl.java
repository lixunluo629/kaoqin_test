...
/* loaded from: poi-ooxml-schemas-3.17.jar:org/openxmlformats/schemas/wordprocessingml/x2006/main/impl/CTHeightImpl.class */
public class CTHeightImpl extends XmlComplexContentImpl implements CTHeight {
    ...
    public void xsetVal(STTwipsMeasure sTTwipsMeasure) {
        synchronized (monitor()) {
            check_orphaned();
            STTwipsMeasure sTTwipsMeasure2 = (STTwipsMeasure) get_store().find_attribute_user(VAL$0);
            if (sTTwipsMeasure2 == null) {
                sTTwipsMeasure2 = (STTwipsMeasure) get_store().add_attribute_user(VAL$0);
            }
    ...
    public STHeightRule xgetHRule() {
        STHeightRule sTHeightRuleFind_attribute_user;
        synchronized (monitor()) {
            check_orphaned();
            sTHeightRuleFind_attribute_user = get_store().find_attribute_user(HRULE$2);
        }
        return sTHeightRuleFind_attribute_user;
    }
    ...
    public void xsetHRule(STHeightRule sTHeightRule) {
        synchronized (monitor()) {
            check_orphaned();
            STHeightRule sTHeightRuleFind_attribute_user = get_store().find_attribute_user(HRULE$2);
            if (sTHeightRuleFind_attribute_user == null) {
                sTHeightRuleFind_attribute_user = (STHeightRule) get_store().add_attribute_user(HRULE$2);
            }
            sTHeightRuleFind_attribute_user.set(sTHeightRule);
        }
    }
