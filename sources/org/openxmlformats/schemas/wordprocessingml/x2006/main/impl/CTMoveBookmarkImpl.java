...
/* loaded from: poi-ooxml-schemas-3.17.jar:org/openxmlformats/schemas/wordprocessingml/x2006/main/impl/CTMoveBookmarkImpl.class */
public class CTMoveBookmarkImpl extends CTBookmarkImpl implements CTMoveBookmark {
    ...
    public void xsetAuthor(STString sTString) {
        synchronized (monitor()) {
            check_orphaned();
            STString sTString2 = (STString) get_store().find_attribute_user(AUTHOR$0);
            if (sTString2 == null) {
                sTString2 = (STString) get_store().add_attribute_user(AUTHOR$0);
            }
    ...
    public STDateTime xgetDate() {
        STDateTime sTDateTimeFind_attribute_user;
        synchronized (monitor()) {
            check_orphaned();
            sTDateTimeFind_attribute_user = get_store().find_attribute_user(DATE$2);
        }
        return sTDateTimeFind_attribute_user;
    }
    ...
    public void xsetDate(STDateTime sTDateTime) {
        synchronized (monitor()) {
            check_orphaned();
            STDateTime sTDateTimeFind_attribute_user = get_store().find_attribute_user(DATE$2);
            if (sTDateTimeFind_attribute_user == null) {
                sTDateTimeFind_attribute_user = (STDateTime) get_store().add_attribute_user(DATE$2);
            }
            sTDateTimeFind_attribute_user.set(sTDateTime);
        }
    }
}
