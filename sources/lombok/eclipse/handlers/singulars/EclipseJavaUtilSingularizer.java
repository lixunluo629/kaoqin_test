...
/* loaded from: lombok-1.16.22.jar:lombok/eclipse/handlers/singulars/EclipseJavaUtilSingularizer.SCL.lombok */
abstract class EclipseJavaUtilSingularizer extends EclipseSingularsRecipes.EclipseSingularizer {
    ...
    protected List<Statement> createJavaUtilSetMapInitialCapacitySwitchStatements(EclipseSingularsRecipes.SingularData data, EclipseNode builderType, boolean mapMode, String emptyCollectionMethod, String singletonCollectionMethod, String targetType) {
        ...
        ((SwitchStatement) switchStatement).statements = (Statement[]) switchContents.toArray(new Statement[switchContents.size()]);
        ((SwitchStatement) switchStatement).expression = getSize(builderType, keyName, true);
    ...
    protected List<Statement> createJavaUtilSimpleCreationAndFillStatements(EclipseSingularsRecipes.SingularData data, EclipseNode builderType, boolean mapMode, boolean defineVar, boolean addInitialCapacityArg, boolean nullGuard, String targetType) {
        ...
        if (addInitialCapacityArg) {
            BinaryExpression binaryExpression = new BinaryExpression(getSize(builderType, varName, nullGuard), EclipseHandlerUtil.makeIntLiteral("0x40000000".toCharArray(), null), 4);
        ...
        if (mapMode) {
            ...
            ifStatement = new ForStatement(new Statement[]{localDeclaration}, new BinaryExpression(new SingleNameReference(iVar, 0L), getSize(builderType, varName, nullGuard), 4), new Statement[]{new PostfixExpression(new SingleNameReference(iVar, 0L), IntLiteral.One, 14, 0)}, pluralnameDotPut, true, 0, 0);
        } else {
