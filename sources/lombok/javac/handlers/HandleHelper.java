...
/* loaded from: lombok-1.16.22.jar:lombok/javac/handlers/HandleHelper.SCL.lombok */
public class HandleHelper extends JavacAnnotationHandler<Helper> {
    ...
    public void handle(AnnotationValues<Helper> annotation, JCTree.JCAnnotation ast, JavacNode annotationNode) {
        ...
        while (it2.hasNext()) {
            JavacNode ch2 = it2.next();
            if (ch2.getKind() == AST.Kind.METHOD && (n = ch2.getName()) != null && !n.isEmpty() && n.charAt(0) != '<') {
                knownMethodNames.add(n);
            }
        }
        Collections.sort(knownMethodNames);
        final String[] knownMethodNames_ = (String[]) knownMethodNames.toArray(new String[knownMethodNames.size()]);
        final Name helperName = annotationNode.toName(PropertiesBeanDefinitionReader.CONSTRUCTOR_ARG_PREFIX + ((JCTree.JCClassDecl) jCStatement).name);
