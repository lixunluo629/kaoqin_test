...
/* loaded from: kernel-7.1.10.jar:com/itextpdf/kernel/xmp/impl/XMPUtilsImpl.class */
public final class XMPUtilsImpl implements XMPConst {
    ...
    public static String catenateArrayItems(XMPMeta xmp, String schemaNS, String arrayName, String separator, String quotes, boolean allowCommas) throws XMPException {
        ...
        if (quotes == null || quotes.length() == 0) {
            quotes = SymbolConstants.QUOTES_SYMBOL;
        }
    ...
    public static void separateArrayItems(XMPMeta xmp, String schemaNS, String arrayName, String catedStr, PropertyOptions arrayOptions, boolean preserveCommas) throws XMPException {
        ...
        while (itemEnd < endPos) {
            ...
            while (itemStart < endPos) {
                ch2 = catedStr.charAt(itemStart);
                charKind = classifyCharacter(ch2);
            ...
            if (itemStart < endPos) {
                if (charKind != 4) {
                    itemEnd = itemStart;
                    while (itemEnd < endPos) {
                        ch2 = catedStr.charAt(itemEnd);
                        charKind = classifyCharacter(ch2);
                        if (charKind != 0 && charKind != 4 && (charKind != 2 || !preserveCommas)) {
                            if (charKind != 1 || itemEnd + 1 >= endPos) {
                                break;
                            }
                            ch2 = catedStr.charAt(itemEnd + 1);
                            int nextKind = classifyCharacter(ch2);
                    ...
                    while (true) {
                        ...
                        ch2 = catedStr.charAt(itemEnd);
                        charKind = classifyCharacter(ch2);
                        if (charKind != 4 || !isSurroundingQuote(ch2, openQuote, closeQuote)) {
                            itemValue = itemValue + ch2;
                        } else {
                            if (itemEnd + 1 < endPos) {
                            ...
                            if (ch2 == nextChar) {
                                itemValue = itemValue + ch2;
                                itemEnd++;
                            } else if (!isClosingingQuote(ch2, openQuote, closeQuote)) {
                                itemValue = itemValue + ch2;
                            } else {
                                itemEnd++;
                                break;
    ...
    private static void appendSubtree(XMPMetaImpl destXMP, XMPNode sourceNode, XMPNode destParent, boolean replaceOldValues, boolean deleteEmptyValues) throws XMPException {
        ...
        if (deleteEmptyValues) {
            if (sourceNode.getOptions().isSimple()) {
                z = sourceNode.getValue() == null || sourceNode.getValue().length() == 0;
            } else {
                z = !sourceNode.hasChildren();
            }
            valueIsEmpty = z;
        }
    ...
    private static int classifyCharacter(char ch2) {
        if (SPACES.indexOf(ch2) >= 0) {
            return 1;
        }
        if (8192 <= ch2 && ch2 <= 8203) {
            return 1;
        }
        if (COMMAS.indexOf(ch2) >= 0) {
            return 2;
        }
        if (SEMICOLA.indexOf(ch2) >= 0) {
            return 3;
        }
        if (QUOTES.indexOf(ch2) >= 0) {
            return 4;
        }
        if (12296 <= ch2 && ch2 <= 12303) {
            return 4;
        }
        if (8216 <= ch2 && ch2 <= 8223) {
            return 4;
        }
        if (ch2 < ' ' || CONTROLS.indexOf(ch2) >= 0) {
            return 5;
        }
        return 0;
    }
    ...
    private static String applyQuotes(String item, char openQuote, char closeQuote, boolean allowCommas) {
        ...
        int i = 0;
        while (i < item.length()) {
            char ch2 = item.charAt(i);
            int charKind = classifyCharacter(ch2);
            if (i == 0 && charKind == 4) {
                break;
            }
            if (charKind == 1) {
    ...
    private static boolean isSurroundingQuote(char ch2, char openQuote, char closeQuote) {
        return ch2 == openQuote || isClosingingQuote(ch2, openQuote, closeQuote);
    }

    private static boolean isClosingingQuote(char ch2, char openQuote, char closeQuote) {
        return ch2 == closeQuote || (openQuote == 12317 && ch2 == 12318) || ch2 == 12319;
    }
}
