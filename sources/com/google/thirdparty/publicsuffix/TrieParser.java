...
@GwtCompatible
/* loaded from: guava-18.0.jar:com/google/thirdparty/publicsuffix/TrieParser.class */
class TrieParser {
    ...
    static ImmutableMap<String, PublicSuffixType> parseTrie(CharSequence encoded) {
        ...
        while (true) {
            ...
            if (idx < encodedLen) {
                iDoParseTrieToBuilder = idx + doParseTrieToBuilder(Lists.newLinkedList(), encoded.subSequence(idx, encodedLen), builder);
            } else {
    ...
    private static int doParseTrieToBuilder(List<CharSequence> stack, CharSequence encoded, ImmutableMap.Builder<String, PublicSuffixType> builder) {
        ...
        if (c != '?' && c != ',') {
            while (idx2 < encodedLen) {
                idx2 += doParseTrieToBuilder(stack, encoded.subSequence(idx2, encodedLen), builder);
