...
/* loaded from: classmate-1.3.4.jar:com/fasterxml/classmate/TypeResolver.class */
public class TypeResolver implements Serializable {
    ...
    public ResolvedType resolve(Type type, Type... typeParameters) throws NegativeArraySizeException {
    ...
    public ResolvedArrayType arrayType(Type elementType) throws NegativeArraySizeException {
    ...
    public ResolvedType resolve(TypeBindings typeBindings, Type jdkType) throws NegativeArraySizeException {
        return _fromAny(null, jdkType, typeBindings);
    }

    public ResolvedType resolveSubtype(ResolvedType supertype, Class<?> subtype) throws UnsupportedOperationException, IllegalArgumentException, NegativeArraySizeException {
        ...
        if (paramCount == 0) {
            ...
            for (int i2 = 0; i2 < paramCount; i2++) {
                resolvedParams[i2] = _fromAny(null, placeholders[i2], bindings);
            }
        ...
        for (int i3 = 0; i3 < paramCount; i3++) {
            ResolvedType t = placeholders[i3].actualType();
            if (t == null) {
                throw new IllegalArgumentException("Failed to find type parameter #" + (i3 + 1) + "/" + paramCount + " for " + subtype.getName());
            }
            typeParams[i3] = t;
        }
    ...
    private ResolvedType _fromAny(ClassStack context, Type mainType, TypeBindings typeBindings) throws NegativeArraySizeException {
    ...
    private ResolvedType _fromClass(ClassStack context, Class<?> rawType, TypeBindings typeBindings) throws NegativeArraySizeException {
        ClassStack context2;
        ResolvedType type;
    ...
    private ResolvedType _constructType(ClassStack context, Class<?> rawType, TypeBindings typeBindings) throws NegativeArraySizeException {
        if (rawType.isArray()) {
    ...
    private ResolvedType[] _resolveSuperInterfaces(ClassStack context, Class<?> rawType, TypeBindings typeBindings) throws NegativeArraySizeException {
    ...
    private ResolvedType _resolveSuperClass(ClassStack context, Class<?> rawType, TypeBindings typeBindings) throws NegativeArraySizeException {
    ...
    private ResolvedType _fromArrayType(ClassStack context, GenericArrayType arrayType, TypeBindings typeBindings) throws NegativeArraySizeException {
    ...
    private ResolvedType _fromWildcard(ClassStack context, WildcardType wildType, TypeBindings typeBindings) throws NegativeArraySizeException {
        return _fromAny(context, wildType.getUpperBounds()[0], typeBindings);
    }

    private ResolvedType _fromVariable(ClassStack context, TypeVariable<?> variable, TypeBindings typeBindings) throws NegativeArraySizeException {
