...
/* loaded from: classmate-1.3.4.jar:com/fasterxml/classmate/TypeBindings.class */
public final class TypeBindings {
    ...
    public static TypeBindings create(Class<?> erasedType, ResolvedType[] types) {
        ...
        if (names.length != types.length) {
            throw new IllegalArgumentException("Can not create TypeBinding for class " + erasedType.getName() + " with " + types.length + " type parameter" + (types.length == 1 ? "" : ExcelXmlConstants.CELL_DATA_FORMAT_TAG) + ": class expects " + names.length);
        }
