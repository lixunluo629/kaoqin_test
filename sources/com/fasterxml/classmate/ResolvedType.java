...
/* loaded from: classmate-1.3.4.jar:com/fasterxml/classmate/ResolvedType.class */
public abstract class ResolvedType implements Type {
    ...
    protected RawField[] _getFields(boolean statics) {
        ...
        Field[] arr$ = this._erasedType.getDeclaredFields();
        for (Field f : arr$) {
    ...
    protected RawMethod[] _getMethods(boolean statics) throws SecurityException {
        ArrayList<RawMethod> methods = new ArrayList<>();
        Method[] arr$ = this._erasedType.getDeclaredMethods();
        for (Method m : arr$) {
    ...
    protected RawConstructor[] _getConstructors() throws SecurityException {
        ArrayList<RawConstructor> ctors = new ArrayList<>();
        Constructor<?>[] arr$ = this._erasedType.getDeclaredConstructors();
