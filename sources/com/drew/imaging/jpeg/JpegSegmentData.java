...
/* loaded from: metadata-extractor-2.10.1.jar:com/drew/imaging/jpeg/JpegSegmentData.class */
public class JpegSegmentData {
    ...
    private List<byte[]> getOrCreateSegmentList(byte segmentType) {
        ...
        if (this._segmentDataMap.containsKey(Byte.valueOf(segmentType))) {
            segmentList = this._segmentDataMap.get(Byte.valueOf(segmentType));
        } else {
    ...
    public void removeSegmentOccurrence(byte segmentType, int occurrence) {
        List<byte[]> segmentList = this._segmentDataMap.get(Byte.valueOf(segmentType));
