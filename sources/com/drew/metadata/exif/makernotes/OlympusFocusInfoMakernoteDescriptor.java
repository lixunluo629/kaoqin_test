...
/* loaded from: metadata-extractor-2.10.1.jar:com/drew/metadata/exif/makernotes/OlympusFocusInfoMakernoteDescriptor.class */
public class OlympusFocusInfoMakernoteDescriptor extends TagDescriptor<OlympusFocusInfoMakernoteDirectory> {
    ...
    public String getFocusDistanceDescription() {
        ...
        if (value == null || value.getNumerator() == 4294967295L || value.getNumerator() == 0) {
            return "inf";
        }
        return (value.getNumerator() / 1000.0d) + " m";
    }
    ...
    public String getExternalFlashZoomDescription() {
        ...
        if (values.length > 1) {
            join = join + SymbolConstants.SPACE_SYMBOL + String.format("%d", Short.valueOf((short) values[1]));
        }
        if (join.equals("0")) {
            return "Off";
        }
