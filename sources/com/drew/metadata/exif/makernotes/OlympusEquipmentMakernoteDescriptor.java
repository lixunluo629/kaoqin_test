...
/* loaded from: metadata-extractor-2.10.1.jar:com/drew/metadata/exif/makernotes/OlympusEquipmentMakernoteDescriptor.class */
public class OlympusEquipmentMakernoteDescriptor extends TagDescriptor<OlympusEquipmentMakernoteDirectory> {
    ...
    public String getLensTypeDescription() throws NumberFormatException {
        ...
        String[] values = str.split(SymbolConstants.SPACE_SYMBOL);
        if (values.length < 6) {
            return null;
        }
        try {
    ...
    public String getMaxApertureAtMinFocalDescription() {
        ...
        return format.format(CalcMaxAperture(value.intValue()));
    }
    ...
    private static double CalcMaxAperture(int value) {
        return Math.pow(Math.sqrt(2.0d), value / 256.0d);
    }
    ...
    @Nullable
    public String getExtenderDescription() throws NumberFormatException {
        ...
        try {
            int num1 = Integer.parseInt(values[0]);
            int num2 = Integer.parseInt(values[2]);
            String extenderType = String.format("%X %02X", Integer.valueOf(num1), Integer.valueOf(num2));
