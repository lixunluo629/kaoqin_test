...
/* loaded from: lombok-1.16.22.jar:com/zwitserloot/cmdreader/CmdReader.SCL.lombok */
public class CmdReader<T> {
    ...
    private void generateShortSummary(String commandName, StringBuilder out) {
        if (commandName != null && commandName.length() > 0) {
            out.append(commandName).append(SymbolConstants.SPACE_SYMBOL);
        }
        ...
        if (sb.length() > 0) {
            out.append("-").append((CharSequence) sb).append(SymbolConstants.SPACE_SYMBOL);
            sb.setLength(0);
        }
        for (ParseItem item3 : this.items) {
            if (!item3.isSeq() && item3.isMandatory() && item3.getShorthand().length() == 0) {
                out.append(ScriptUtils.DEFAULT_COMMENT_PREFIX).append(item3.getFullName()).append("=val ");
            }
        }
        ...
        out.append(ScriptUtils.FALLBACK_STATEMENT_SEPARATOR);
    }
    ...
    private void generateArgHelp(int maxFullName, int maxShorthand, ParseItem item, StringBuilder out) {
        ...
        for (char c : arr$) {
            if (sh.length() > 0) {
                sh.append(SymbolConstants.SPACE_SYMBOL);
            }
            sh.append("-").append(c);
        }
        ...
        if (description.length() == 0 || description.length() < left) {
            out.append(description).append(ScriptUtils.FALLBACK_STATEMENT_SEPARATOR);
            return;
        }
    ...
    */
    public T make(java.lang.String r5) throws com.zwitserloot.cmdreader.InvalidCommandLineException, java.lang.IllegalArgumentException {
        ...
        Lbd:
            ...
            return r0
        */
        throw new UnsupportedOperationException("Method not decompiled: com.zwitserloot.cmdreader.CmdReader.make(java.lang.String):java.lang.Object");
    }
    ...
    public T make(String[] in) throws IllegalAccessException, NoSuchMethodException, SecurityException, InvalidCommandLineException, IllegalArgumentException {
        ...
        int i = 0;
        while (i < in.length) {
            if (in[i].startsWith(ScriptUtils.DEFAULT_COMMENT_PREFIX)) {
