...
/* loaded from: mysql-connector-java-5.1.48.jar:com/mysql/jdbc/Security.class */
public class Security {
    ...
    static byte[] createKeyFromOldPassword(String passwd) throws NoSuchAlgorithmException {
        int[] salt = getSaltFromPassword(makeScrambledPassword(passwd));
        ...
        if (usingNewPasswords) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                for (int t = 3; t >= 0; t--) {
                    int i2 = pos;
                    pos++;
                    binaryPassword[i2] = (byte) (val & 255);
                    val >>= 8;
                }
            }
    ...
    private static int[] getSaltFromPassword(String password) {
        ...
        while (pos < length) {
            ...
            for (int i2 = 0; i2 < 8; i2++) {
                int i3 = pos;
                pos++;
                val2 = (val2 << 4) + charVal(password.charAt(i3));
            }
            int i4 = resultPos;
            resultPos++;
            result[i4] = val2;
    ...
    public static byte[] scramble411(String password, String seed, String passwordEncoding) throws NoSuchAlgorithmException, UnsupportedEncodingException {
        ...
        md.reset();
        byte[] seedAsBytes = StringUtils.getBytes(seed, NTLM.DEFAULT_CHARSET);
    ...
    public static byte[] scrambleCachingSha2(byte[] password, byte[] seed) throws NoSuchAlgorithmException, DigestException {
        try {
